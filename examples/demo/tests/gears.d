...
class Gears : Test
{
    ...
    override void Step(Settings* settings)
    {
        ...
        g_debugDraw.DrawString(5, m_textLine, format("theta1 + %4.2f * theta2 = %4.2f", cast(float)ratio, cast(float)value));
        m_textLine += DRAW_STRING_NEW_LINE;

        ratio = m_joint5.GetRatio();
        value = m_joint2.GetJointAngle() + ratio * m_joint3.GetJointTranslation();
        g_debugDraw.DrawString(5, m_textLine, format("theta2 + %4.2f * delta = %4.2f", cast(float)ratio, cast(float)value));
        m_textLine += DRAW_STRING_NEW_LINE;
    }
